...
func _ready() -> void:
	...
	if Engine.is_editor_hint():
		set_process(false)
	day_of_year = Time.get_unix_time_from_datetime_dict(Time.get_datetime_dict_from_system()) / 86400 % DAYS_IN_YEAR

func _process(_delta: float) -> void:
	#if Engine.is_editor_hint(): return # We don't want a time lapse in the editor
	day_time = Game.time
